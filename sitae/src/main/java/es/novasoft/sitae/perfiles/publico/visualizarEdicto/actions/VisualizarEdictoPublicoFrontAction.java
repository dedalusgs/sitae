package es.novasoft.sitae.perfiles.publico.visualizarEdicto.actions;
...
public class VisualizarEdictoPublicoFrontAction extends ActionBase {
		...
		try {
			...
			if (request.getParameter("accion") != null && !request.getParameter("accion").equals("")) {
				...
				if (accion.equals("edictosVigor")) {
					...
					request.getSession().setAttribute("accionPublico", "edictosVigor");
				}
				if (accion.equals("historicoEdictos")) {
					formulario.setTitulo("");
					formulario.setDescripcion("");
					formulario.setFechaPublicacionFin("");
					formulario.setFechaPublicacionInicio("");
					...
					formulario.setNumeroExpediente("");
					formulario.setOpcionOrganismoExterno(-1);
					request.getSession().setAttribute("accionPublico", "historicoEdictos");
					request.getSession().setAttribute("consultaHistorico", "S");
				}
				if (accion.equals("busquedaAvanzada")) {
					request.getSession().setAttribute("accionPublico", "busquedaAvanzada");
				}
			} else {
