...
<%@page import="java.util.Locale, org.apache.struts.Globals"%>
	...
	<script type="text/javascript">
		...
		<h3><bean:message key="visualizarListaOrganismo.titulo" bundle="resources"/></h3>
					
	  					 <display:table export="true" name="${VisualizarOrganismoForm.nuevalistaOrganismos}"  id="nuevalistaOrganismos" pagesize="10" sort="list"
	  					 requestURI="./VisualizarOrganismoFrontAction.do">
						
							<c:set var="cifProperty"><bean:message key="cabecera.cif" bundle="resources" /></c:set>
								   
							<display:column class="listado_representantes" headerClass="cabecera_tabla ancho-60" property="cif" title="${cifProperty}"/>		   
							...
							<% if (locale!=null){ %>	
									<%if(locale.getLanguage().equals("va")){%>				  			
			  							<display:column class="listado_representantes" headerClass="cabecera_tabla" property="nombreVa" title="${organismoProperty}"/>
									<%}else {%>
							...
							<display:column media="html" class="listado_representantes" headerClass="cabecera_tabla ancho-20">
								...
								<center>
									<html:link action="/ModificarOrganismoFrontAction.do" paramId="idOrganismoSelecionado" paramName="nuevalistaOrganismos" paramProperty="idOrg">			
							...
							<display:column media="html" class="listado_expedientes" headerClass="cabecera_tabla ancho-20">					
								...
								<center>
									<html:link action="/BajaOrganismoDo.do" paramId="idOrganismoSelecionado" paramName="nuevalistaOrganismos" paramProperty="idOrg">
							...
							<display:column media="html" class="listado_expedientes" headerClass="cabecera_tabla ancho-20">					
									...
									<center>
										<html:link action="/MuestraInformacionOrganismoFrontAction.do" paramId="idOrganismoSelecionado" paramName="nuevalistaOrganismos" paramProperty="idOrg">
	  				...
	  				<br/>
	  				<html:form action="/AltaOrganismoFrontAction.do" method="post" enctype="multipart/form-data">
