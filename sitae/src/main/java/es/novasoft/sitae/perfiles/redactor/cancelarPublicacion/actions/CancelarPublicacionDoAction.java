package es.novasoft.sitae.perfiles.redactor.cancelarPublicacion.actions;
...
public class CancelarPublicacionDoAction extends ActionBase {
	...
	protected ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (idEdicto != null && !idEdicto.equals("")) {
				...
				if (edicto.getEstado().getIdEstado().equals(Constantes.PENDIENTE_VALIDACION) || edicto.getEstado().getIdEstado().equals(Constantes.PENDIENTE_FIRMA)) {
					...
					while (iterador.hasNext()) {
						...
						res = correo.enviarCorreo(host, remitente, interesado.getEmail(), asunto, texto, adjunto, edicto.getNombrePdfAdjunto());
						// if (res == 0)
						// throw new Exception();
					}
			...
			e.printStackTrace();
			log.error("Error en CancelarPublicacionDoAction", e);
