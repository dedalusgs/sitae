...
<html:form
	action="/RelacionarPublicacionesFrontAction.do?accion=busquedaAvanzada&filtrar=si"
	...
	<fieldset>
		...
		<div class="detalle">
			...
			<html:text name="RelacionarPublicacionesForm" property="titulo"
				maxlength="80" />
		</div>
		...
		<div class="detalle">
			...
			<html:text name="RelacionarPublicacionesForm" property="descripcion"
				maxlength="255" />
		</div>
		<div class="detalle">
		...
		<div class="detalle">
			...
			<html:text maxlength="10" name="RelacionarPublicacionesForm"
...
Calendar.setup( {
	...
	onSelect   : function() { this.hide() }
		...
		<div class="detalle">
			...
			<logic:equal name="RelacionarPublicacionesForm" property="historico"
				value="true">
				<input class="check" type="checkbox" name="historico" checked />
			</logic:equal>

			<logic:notEqual name="RelacionarPublicacionesForm"
	...
	<fieldset>
		...
		<logic:notEmpty name="RelacionarPublicacionesForm"
			property="listaRelacionados">

			<display:table
				name="${RelacionarPublicacionesForm.listaRelacionados}"
				sort="external" defaultsort="1" id="nuevaListaRelacionados"
				...
				<display:column class="listado_representantes" media="html"
					...
					<bean:write name="nuevaListaRelacionados" property="titulo" />
					<br />
					<div id="setDescripcion${nuevaListaPublicados.id}">
						<bean:write name="nuevaListaRelacionados"
							property="descripcionCorta" />
					</div>

					<script type="text/javascript">
$(function() {
	$('#setDescripcion${nuevaListaRelacionados.id}').tooltip( {
		delay : 0,
		showURL : false,
		bodyHandler : function() {
			return $("#textDescripcion${nuevaListaRelacionados.id}").text();
		}
	});
});
</script>

					<div id="textDescripcion${nuevaListaRelacionados.id}"
						style="display: none;">
						<bean:write name="nuevaListaRelacionados" property="descripcion" />
					</div>
				...
				<c:if test="${(nuevaListaRelacionados.idEstado=='6')}">
				...
				<c:set var="eliminarRelacionEdictoProperty">
					<bean:message key="muestraInformacionEdicto.relacionar.eliminar"
						bundle="resources" />
				</c:set>
				...
				<display:column media="html" class="listado_expedientes"
					...
					<center>
						<html:link
							href="RelacionarPublicacionesFrontAction.do?accion=baja&id=${nuevaListaRelacionados.id }">
							<img title="${eliminarRelacionEdictoProperty}"
								src="${pageContext.request.contextPath}/images/ico-darBaja.jpg"
								alt="${eliminarRelacionEdictoProperty}" />
						</html:link>
					</center>
				</display:column>
			</display:table>
	...
	<logic:notEmpty name="RelacionarPublicacionesForm"
		...
		<display:table name="${RelacionarPublicacionesForm.listaPublicados}"
			sort="external" defaultsort="1" id="nuevaListaPublicados"
			pagesize="10" partialList="true" size="totalTablaEdictos"
			requestURI="./RelacionarPublicacionesFrontAction.do">
...
$(function() {
	$('#setDescripcion${nuevaListaPublicados.id}').tooltip( {
		...
		bodyHandler : function() {
			...
			<c:set var="descargarEdictoProperty">
				<bean:message key="muestraInformacionEdicto.relacionar"
					bundle="resources" />
			</c:set>
			...
			<display:column media="html" class="listado_expedientes"
				...
				<center>
					<html:link
						href="RelacionarPublicacionesFrontAction.do?accion=alta&id=${nuevaListaPublicados.id }">
	...
	<logic:empty name="RelacionarPublicacionesForm"
		property="listaPublicados">
...
<div class="botonera border">
	...
	<html:form action='/RelacionarPublicacionesDo.do'>
		<html:hidden property="accion"
			value='<%=(String) session.getAttribute("origenRelacion")%>' />
		<html:submit>
