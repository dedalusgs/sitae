package es.novasoft.sitae.perfiles.adminGlobal.altaOrganismoExterno.forms;
...
public class AltaOrganismoExternoForm extends FormBase {
	...
	public AltaOrganismoExternoForm() {
		...
		if (organismoExterno.getCif().equals("")) {
			...
			if (organismoExterno.getCif().length() >= Constantes.TAMANIO_MAX) {
				...
				if (!ValidatorUtils.esCIF(organismoExterno.getCif())) {
					...
					try {
						listaOrganismo = organismoExternoService
