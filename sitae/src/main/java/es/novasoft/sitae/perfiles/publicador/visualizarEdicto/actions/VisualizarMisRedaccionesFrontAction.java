...
import es.novasoft.sitae.perfiles.publicador.visualizarEdicto.forms.VisualizarMisRedaccionesForm;

public class VisualizarMisRedaccionesFrontAction extends ActionBase {
	...
	protected ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			VisualizarMisRedaccionesForm formulario = (VisualizarMisRedaccionesForm) form;
			...
			request.getSession().setAttribute("VisualizarEdictoForm", formulario);
			...
			e.printStackTrace();
			LOGGER.error("Error en VisualizarMisRedaccionesFrontAction", e);
			return forward(request, mapping, ActionBase.ERROR_KEY);
		}
		return forward(request, mapping, ActionBase.SUCCESS_KEY);
	}
	
	public VisualizarMisRedaccionesForm rellenaFormulario(VisualizarMisRedaccionesForm formulario, String cif, HttpServletRequest request, String lenguaje) throws Exception {
		...
		List listaEstados = RellenaFormularioActionUtil.RellenaEstadosSinRetirado();
		...
		Usuario usuario = (Usuario) usuarioService.findByNumeroDocumento(numDocumento).get(0);
		...
		int totalEdictos = edictoService.findByOrganismoRedactorSinEstadoHistoricoCount(organismo.getIdOrg(), usuario.getIdUsuario());
		...
		if (request.getParameter(TableTagParameters.PARAMETER_EXPORTING) != null) {
			listaEdictos = edictoService.findByOrganismoRedactorSinEstadoHistorico(organismo.getIdOrg(), usuario.getIdUsuario(), 1, totalEdictos);
		} else {
			// Obtener el listado de elementos de BBDD específicos para la
			// página;
			listaEdictos = edictoService.findByOrganismoRedactorSinEstadoHistorico(organismo.getIdOrg(), usuario.getIdUsuario(), paginaActual, 10);
		}
		...
		while (it.hasNext()) {
			...
			String cifOrganismo = edicto.getOrganismo().getCif();
			String dniRedactor = edicto.getRedactor().getNumDocumento();
			Integer idCentro = edicto.getCentro().getIdCentro();
			boolean publicadorCentroProcencia = false;
			
			List listaCentroUsuPerf = relacionUsuOrgCentroPerfService.findByCentroUsuPerf(String.valueOf(idCentro), dniRedactor, String.valueOf(Constantes.PUBLICADOR));
			
			List listaCentroUsuPerfLocal = relacionUsuOrgCentroPerfService.findByOrgUsuPerf(cifOrganismo, dniRedactor, String.valueOf(Constantes.ADMIN_LOCAL));
			
			if (!listaCentroUsuPerf.isEmpty() || !listaCentroUsuPerfLocal.isEmpty()) {
				publicadorCentroProcencia = true;
			}
			
			objetoEdictoVisualizar.setPublicadorCentroProcencia(publicadorCentroProcencia);
	...
	public VisualizarMisRedaccionesForm filtrar(VisualizarMisRedaccionesForm formulario, String cif, HttpServletRequest request, String lenguaje) throws Exception {
		...
		// Contar el total de elementos
		int totalEdictos = edictoService.findByFiltroSinEstadoRetiradoCount(formulario.getFechaRedaccion(), organismo.getIdOrg(), formulario.getOpcionCentro(),
		        formulario.getOpcionOrganismoExterno(), formulario.getOpcionEstado(), formulario.getOpcionTipoEdicto(), usuario.getIdUsuario(), Constantes.USUARIO_REDACTOR,
		        formulario.getTitulo(), formulario.getNumeroExpediente(), lenguaje);
		
		request.setAttribute("totalTablaEdictos", totalEdictos);
		// Obtener el listado de elementos de BBDD específicos para la página;
		List listaEdictos;
		if (request.getParameter(TableTagParameters.PARAMETER_EXPORTING) != null) {
			listaEdictos = edictoService.findByFiltroSinEstadoRetirado(formulario.getFechaRedaccion(), organismo.getIdOrg(), formulario.getOpcionCentro(),
			        formulario.getOpcionOrganismoExterno(), formulario.getOpcionEstado(), formulario.getOpcionTipoEdicto(), usuario.getIdUsuario(), Constantes.USUARIO_REDACTOR,
			        formulario.getTitulo(), formulario.getNumeroExpediente(), lenguaje, 1, totalEdictos);
		} else {
			listaEdictos = edictoService.findByFiltroSinEstadoRetirado(formulario.getFechaRedaccion(), organismo.getIdOrg(), formulario.getOpcionCentro(),
			        formulario.getOpcionOrganismoExterno(), formulario.getOpcionEstado(), formulario.getOpcionTipoEdicto(), usuario.getIdUsuario(), Constantes.USUARIO_REDACTOR,
			        formulario.getTitulo(), formulario.getNumeroExpediente(), lenguaje, paginaActual, 10);
		}
		List listaPublicados = new ArrayList<ObjetoEdictoVisualizar>();
