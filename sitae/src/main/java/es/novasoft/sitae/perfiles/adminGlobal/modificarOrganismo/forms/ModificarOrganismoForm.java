package es.novasoft.sitae.perfiles.adminGlobal.modificarOrganismo.forms;
...
public class ModificarOrganismoForm extends FormBase {
	...
	public ActionErrors validate(ActionMapping mapping,
		...
		if (organismo.getCif().equals("")) {
			...
			if (organismo.getCif().length() >= Constantes.TAMANIO_MAX) {
				...
				if (!ValidatorUtils.esCIF(organismo.getCif())) {
					...
					try {
						listaOrganismos = organismoService.findByCif(organismo
								.getCif());
						if (!listaOrganismos.isEmpty()
								&& !cifAntiguo.equals(organismo.getCif())) {
		...
		try {
			listaOrganismo2 = organismoService.findByCodigo(organismo.getCodigo());
		...
		if (organismo.getAliasSello().equals("")) {
			errors.add(null, new ActionMessage("errors.required", LenguajeUtil
					.getMensaje(request, "datosorganismo.aliasSello")));
		}
		/*
