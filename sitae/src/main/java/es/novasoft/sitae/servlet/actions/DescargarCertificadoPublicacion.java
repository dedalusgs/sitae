 ...
 */
public class DescargarCertificadoPublicacion extends DownloadAction {
	...
	protected StreamInfo getStreamInfo(ActionMapping mapping, ActionForm form,
		...
		if (edicto != null
				&& edicto.getEstado().getIdEstado() == Constantes.PUBLICADO) {
			...
			try {
				...
				certificadoPublicacion = client
						.getPDFCertificado(urlCertificado);

				OutputStream out = response.getOutputStream();

				byte[] buf = new sun.misc.BASE64Decoder()
						.decodeBuffer(certificadoPublicacion);
				...
				response
						.setHeader(
								"Content-Disposition",
								("attachment;filename=\"Certificado Publicacion - " + edicto
										.getCodigo())
										+ ".pdf\"");
